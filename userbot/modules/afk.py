...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if mention.sender_id not in USERS or chat_title not in USERS:
            if AFKREASON:
                await mention.reply(
                    f"**Aku Off Dulu Sebentar Yhaa!.** (Since: {afk_str})"
                    f"\nReason: `{AFKREASON}`."
                )
            else:
                await mention.reply(
                    f"**Aku Off Dulu Sebentar Yhaa!.** (Since: {afk_str})"
                    "\n**Ehh Sabar Dikit Napa Si! 😭.**"
                )
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(
                            "**Aku Off Dulu Sebentar Yhaa!.** (Since: {afk_str})"
                            f"\nReason: `{AFKREASON}`."
                        )
                    else:
                        await sender.reply(
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("**Aku Kembali Online, Ada Apa Yhaa Cariin Aku?!**")
...
CMD_HELP.update(
    {
        > .off [Alasan Opsional]\ nPenggunaan: Menetapkan Anda sebagai afk. \ nBalas kepada siapa saja yang menandai / PMAnda memberi tahu mereka bahwa Anda AFK (alasan).\ n \

n> .unoff\ nPenggunaan: Mematikan Anda dari mode AFK
    }
)
