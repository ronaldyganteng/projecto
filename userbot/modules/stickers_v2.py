...
async def _(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
        await event.edit("`Mohon Balas Ke Sticker Sayang`")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await event.edit("`Mohon Balas Ke Sticker Sayang`")
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response.text.startswith("I understand only stickers"):
            await event.edit("`Maaf Kamu, Aku Tidak Bisa Mengubah Ini Menjadi Gambar, Periksa Kembali Apakah Itu Sticker Animasi?`")
        else:
            response = conv.wait_event(
                events.NewMessage(
                    incoming=True,
...
async def sticker_to_png(sticker):
    ...
    if not img.document:
        await sticker.edit("`Maaf Kamu, Ini Bukan Sticker`")
        return False
